# Fast-Canada
> ⚠️ Проект является fork-версией исходного репозитория. Исходный репозиторий: [<ссылка>](https://github.com/Kasssian/Fast-Canada)
> Основной вклад — реализация e-commerce логики (корзина, заказы, email).
...
Проект был взят как база и расширен. Лично мной реализовано:

- 🛒 Подсистема корзины (session-based storage, cart manager, валидация)
- 📦 Оформление заказа
- 📧 Email-уведомления при создании заказа
- 🧩 Расширение бизнес-логики e-commerce
